...
async def test_bulk_delete_removes_all_requested_books_and_writes_history() -> None:
	...
	deleted = await use_case.execute(
		BulkDeleteBooksInput(book_ids=[book1.id, book2.id], library_id=library_id, changed_by=changed_by)
	)
...
async def test_bulk_delete_fails_all_or_nothing_when_one_id_missing() -> None:
	...
	with pytest.raises(LookupError):
		await use_case.execute(
			BulkDeleteBooksInput(book_ids=[book1.id, missing_id], library_id=library_id, changed_by=uuid4())
		)
...
@pytest.mark.asyncio
async def test_bulk_delete_fails_all_or_nothing_when_one_id_belongs_to_another_library() -> None:
	library_id = uuid4()
	...
	with pytest.raises(PermissionError):
		await use_case.execute(
			BulkDeleteBooksInput(book_ids=[own_book.id, foreign_book.id], library_id=library_id, changed_by=uuid4())
		)
...
async def test_bulk_delete_dedupes_repeated_ids() -> None:
	...
	use_case = BulkDeleteBooksUseCase(book_repo, history_repo)
	deleted = await use_case.execute(
		BulkDeleteBooksInput(book_ids=[book.id, book.id], library_id=library_id, changed_by=uuid4())
	)
