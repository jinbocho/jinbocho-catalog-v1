...
from app.application.use_cases.ingestion.google_books_mapper import volume_to_metadata
...
class SearchBooksUseCase:
	...
	async def execute(self, title: str | None, author: str | None, max_results: int = 10) -> list[dict[str, Any]]:
		...
		if results:
			return results
		# Google Books has a low unauthenticated quota — fall back to Open Library's
		# free-text search so the feature still works without an API key.
	...
	async def _search_google_books(
		...
		try:
			response = await self._http_client.get(f"{settings.google_books_url}/volumes", params=params)
	...
	async def _search_open_library(
		...
		try:
			response = await self._http_client.get(f"{settings.open_library_url}/search.json", params=params)
			response.raise_for_status()
			data = response.json()
		except httpx.HTTPError:
			return []
