...
class UpdateReadingStatusUseCase:
	...
	async def execute(self, inp: UpdateReadingStatusInput) -> OwnedBook:
		...
		old_status = book.reading_status_for(
			await self._read_repo.is_read(book.id, inp.changed_by),
			await self._abandonment_repo.is_abandoned(book.id, inp.changed_by),
		)

		if inp.reading_status == ReadingStatus.READING:
			# Claiming the single physical copy — shared across the library by nature.
			# Picking a book back up is a fresh attempt: clear any abandoned mark.
			book.current_reader_id = inp.changed_by
			await self._abandonment_repo.remove(book.id, inp.changed_by)
		else:
			# "Read"/"to_read"/"abandoned" are personal: they only ever change
			# whether *this* caller has read/abandoned it, never the whole
			# library's view. Mutually exclusive with each other.
		...
		# copy" now — "read"/"abandoned" live exclusively in their own tables.
		...
		new_status = saved.reading_status_for(
			await self._read_repo.is_read(saved.id, inp.changed_by),
			await self._abandonment_repo.is_abandoned(saved.id, inp.changed_by),
		)
