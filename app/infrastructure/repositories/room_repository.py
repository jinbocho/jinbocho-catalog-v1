...
class SQLAlchemyRoomRepository(RoomRepository):
	...
	async def find_all_by_library(self, library_id: UUID, limit: int = 50, offset: int = 0) -> list[Room]:
		result = await self._session.execute(
			select(RoomModel)
			.where(RoomModel.library_id == library_id)
	...
	async def find_by_name(self, library_id: UUID, name: str) -> Room | None:
		result = await self._session.execute(
			select(RoomModel).where(RoomModel.library_id == library_id, RoomModel.name == name)
		)
	...
	async def delete_all_by_library(self, library_id: UUID) -> None:
		await self._session.execute(sa_delete(RoomModel).where(RoomModel.library_id == library_id))
		await self._session.flush()
